...
export default function ChatList() {
    ...
    const handleEditSave = () => {
        setChatItems(prevItems =>
            prevItems.map(item =>
                item.id === editModal.chat.id ? { ...item, title: editModal.chat.title } : item
            )
        );
    ...
    const handleDeleteConfirm = () => {
        setChatItems(prevItems => prevItems.filter(item => item.id !== deleteModal.chat.id));
            ...
            <div className="space-y-4">
                {filteredItems.length > 0 ? (
                    filteredItems.map((item, index) => (
                        <div key={item.id} className="flex items-center p-3 bg-white border rounded-2xl shadow-md">
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-4 rounded-lg shadow-lg w-96">
                        ...
                        <input
                            ...
                            onChange={(e) => setEditModal({ ...editModal, chat: { ...editModal.chat, title: e.target.value } })}
                            className="w-full p-2 border rounded mb-4"
                        />
                        <div className="flex justify-end space-x-2">
                            <button className="px-4 py-2 bg-gray-300 rounded" onClick={() => setEditModal({ open: false, chat: null })}>
                                Cancel
                            </button>
            ...
            {deleteModal.open && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-4 rounded-lg shadow-lg w-96">
                        ...
                        <p className="mb-4">Are you sure you want to delete chat <strong>{deleteModal.chat.title}</strong>?</p>
                        <div className="flex justify-end space-x-2">
                            <button className="px-4 py-2 bg-gray-300 rounded" onClick={() => setDeleteModal({ open: false, chat: null })}>
                                Cancel
                            </button>
                            <button className="px-4 py-2 bg-red-500 text-white rounded" onClick={handleDeleteConfirm}>
                                Confirm
                            </button>
                        </div>
                    </div>
